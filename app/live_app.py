...
from app.live_price import render_live_price
...
def _data(symbol: str, period: str, interval: str):
    try:
...
def _chart(x, sig, position=None):
    ...
    fig.update_layout(height=540, template="plotly_dark", paper_bgcolor="#080b10", plot_bgcolor="#080b10", margin=dict(l=8,r=8,t=25,b=8), xaxis_rangeslider_visible=False, hovermode="x unified", legend=dict(orientation="h", y=1.02, x=0))
    return fig
...
def render_app():
    ...
    with st.sidebar:
        ...
        auto = st.checkbox("AUTO PAPER TRADING", False, help="Paper order processing is bar-based. Live price itself updates in the browser every second without refreshing this page.")
    ...
    if df is None or df.empty:
        st.error(f"No market data for {symbol}. Try 1d + 1mo.")
        return
    x = add_indicators(df)
    ...
    prev = float(x.Close.iloc[-2]) if len(x) > 1 else last
    ...
    st.markdown(f"<div class='hero'><span class='live'><span class='dot'></span>LIVE PRICE · BROWSER 1 SECOND UPDATE · PAPER ONLY</span><div class='brand'>ALGO TRADING PRO</div><div class='sub'>{name} · dashboard loaded {_now()} · only the price ticker updates continuously</div></div>", unsafe_allow_html=True)

    a,b,c,d,e = st.columns(5)
    with a:
        render_live_price(symbol, last)
    ...
    with left:
        st.markdown("<div class='section'>Market Chart · static until manual dashboard refresh</div>", unsafe_allow_html=True)
    ...
    if pos:
        p1,p2,p3,p4,p5=st.columns(5); p1.metric("POSITION",pos.side); p2.metric("ENTRY",_money(pos.entry)); p3.metric("LAST SNAPSHOT",_money(last)); p4.metric("STOP",_money(pos.stop)); p5.metric("TARGET",_money(pos.target))
        st.markdown(f"<div class='order'><div class='small'>OPEN PAPER POSITION · {pos.opened_at}</div><div class='orderprice'>{pos.side} × {pos.qty:,} · Entry {_money(pos.entry)}</div><div class='small'>Position remains session-based. Live price ticker is browser-updated.</div></div>", unsafe_allow_html=True)
    else:
        st.info("FLAT · A qualifying signal can create a paper position when AUTO PAPER TRADING is enabled.")

    st.caption("Live ticker: browser-side 1-second polling, so the Streamlit page, chart, signal and layout do NOT refresh every second. Yahoo Finance may delay or throttle data; unchanged prices can legitimately remain unchanged.")
