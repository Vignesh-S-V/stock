...
SYMBOL_ALIASES = {**{k.upper(): v for k, v in INDEX_UNIVERSE.items()}, **{k.upper(): v for k, v in POPULAR_STOCKS.items()}}
...
def model_signal(x: pd.DataFrame, symbol: str, strategy: str, threshold: float, reward_r: float = 2.0):
    ...
    if ml.action in {"BUY", "SELL"}:
        ...
        if ml.action == "BUY" and not technical.stop < technical.entry < technical.target:
            technical.action = "HOLD"; technical.stop = technical.target = float("nan"); technical.risk_reward = float("nan"); technical.reasons.append("Invalid BUY price geometry; trade suppressed.")
        elif ml.action == "SELL" and not technical.target < technical.entry < technical.stop:
            technical.action = "HOLD"; technical.stop = technical.target = float("nan"); technical.risk_reward = float("nan"); technical.reasons.append("Invalid SELL price geometry; trade suppressed.")
    else:
        technical.stop = technical.target = float("nan"); technical.risk_reward = float("nan")
...
def exit_price_for(sig):
    return sig.target if sig.action in {"BUY", "SELL"} else float("nan")

def sell_price_for(sig):
    return sig.target if sig.action == "BUY" else (sig.entry if sig.action == "SELL" else float("nan"))
...
async def refresh_index(name: str, ticker: str, threshold: float, reward_r: float = 2.0) -> None:
    ...
    try:
        value = await asyncio.to_thread(index_decision, name, ticker, threshold, reward_r); _index_cache[name] = (time.time(), value)
    finally:
        async with _index_refresh_lock: _index_refreshing.discard(name)
...
async def refresh_stale_indices(threshold: float, reward_r: float = 2.0) -> None:
    ...
    for name in INDEX_DISPLAY_ORDER:
        ...
        if cached is None or now - cached[0] >= INDEX_CACHE_TTL: jobs.append(asyncio.create_task(refresh_index(name, INDEX_UNIVERSE[name], threshold, reward_r)))
...
def execute_paper(account: PaperAccount, x: pd.DataFrame, symbol: str, strategy: str, risk_pct: float, brokerage_pct: float, reward_r: float, threshold: float, strict: bool, auto: bool, news: dict[str, Any]):
    ...
    ml_ready = bool(ml and ml.trained)
    if strict:
...
async def websocket_endpoint(ws: WebSocket):
    ...
    try:
        while True:
            ...
            requested_symbol = str(config["symbol"]); resolved_symbol = resolve_symbol(requested_symbol)
            x, price = await asyncio.to_thread(market_snapshot, resolved_symbol)
            if x is None or price is None:
                payload = {"type": "error", "message": f"Live market data unavailable for {requested_symbol} ({resolved_symbol})", "timestamp": time.time()}
            else:
