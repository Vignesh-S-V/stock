...
def market_snapshot(symbol: str) -> tuple[pd.DataFrame | None, float | None]:
    ...
    if cached and now - cached[0] < MARKET_CACHE_TTL:
        return cached[1], cached[2]
    d = fetch_live_1m(symbol, "5d")
    if d is None or d.empty:
        return None, None
    enriched = add_indicators(d)
    price = float(d.Close.iloc[-1])
    _market_cache[symbol] = (now, enriched, price)
    return enriched, price
...
def execute_paper(account: PaperAccount, x: pd.DataFrame, symbol: str, strategy: str, risk_pct: float, brokerage_pct: float, reward_r: float, threshold: float, strict: bool, auto: bool) -> dict[str, Any]:
    ...
    if account.position:
        ...
        if exit_reason:
            ...
            account.realized += pnl - fee; account.cash += pnl - fee
            event = f"{p.side} CLOSED · {exit_reason} · P/L ₹{pnl - fee:,.2f}"; account.position = None
    if account.position is None and auto and qualified and sig.action in {"BUY", "SELL"}:
        ...
        if qty > 0:
            ...
            fee = abs(qty * price) * brokerage_pct / 100; account.cash -= fee
...
@app.get("/universe")
def universe() -> dict[str, dict[str, str]]: return {"indices": INDEX_UNIVERSE, "stocks": POPULAR_STOCKS}
    ...
    try:
        while True:
            ...
            if x is None or price is None:
                ...
                result["type"] = "tick"; result["symbol"] = config["symbol"]; result["indices"] = dict(zip(INDEX_DISPLAY_ORDER, index_results)); payload = result
