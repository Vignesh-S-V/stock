...
function App() {
  ...
  const testIndex = d => {
    ...
    if (!mlReady || !Number.isFinite(dConfidence) || dConfidence < Number(config.threshold) || !['BUY', 'SELL'].includes(dAction)) {
      setError(`${name}: PAPER TRADE NOT OPENED — ML signal is not qualified (${dAction}, ${Number.isFinite(dConfidence) ? fmt(dConfidence, 1) + '%' : 'no confidence'}).`)
      return
    }
  ...
  return <main className="app">
    ...
    <section className="mini-grid">{indices.map((d, i) => { const dAction = d?.decision || 'HOLD'; const dConfidence = Number(d?.confidence); const qualified = mlReady && Number.isFinite(dConfidence) && dConfidence >= Number(config.threshold) && ['BUY', 'SELL'].includes(dAction); return <article key={d?.name || i} className="mini-card"><div className="mini-head"><div><span className="eyebrow">INDEX</span><h3>{d?.name || INDEXES[i]}</h3></div><Pill tone={d?.available ? 'live' : 'muted'}>{d?.available ? 'LIVE' : 'OFFLINE'}</Pill></div><strong>₹{fmt0(d?.price)}</strong><div className={'mini-decision ' + cls(dAction)}>{dAction}</div><div className="mini-line"><span>ML confidence</span><b>{d?.confidence ?? '—'}%</b></div><div className="mini-line"><span>Paper status</span><b className={qualified ? 'bull-text' : 'bear-text'}>{qualified ? `${dAction} READY` : 'NOT OPENED'}</b></div><div className="mini-line"><span>Target</span><b>₹{fmt0(d?.target)}</b></div><button disabled={!qualified} onClick={() => testIndex(d)}>{qualified ? `TEST & PAPER ${dAction}` : 'TEST SIGNAL — NO TRADE'}</button></article> })}</section>
