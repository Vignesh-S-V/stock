  ...
  return (
    <article className="index-card">
      <div className="index-head">
        <div><small>{data?.name || 'INDEX'}</small><strong>₹{fmt(data?.price)}</strong></div>
        <span className={'decision ' + cls(decision)}>{decision}</span>
      </div>
      <div className="index-meta">
        <span>MODEL {data?.confidence ?? '—'}%</span>
        <span>ENTRY ₹{fmt(data?.entry)}</span>
      ...
      {option?.available ? (
        <div className="option-box">
          ...
          <div className="option-grid">
            <div><small>LIVE PREMIUM</small><b>₹{fmt(option.premium)}</b></div>
            <div><small>BUY</small><b>₹{fmt(option.buy_price)}</b></div>
            <div><small>SELL / TARGET</small><b>₹{fmt(option.target_price)}</b></div>
        ...
        <div className="option-unavailable">{decision === 'HOLD' ? 'No option trade — model is HOLD.' : (option?.reason || 'Live option premium unavailable.')}</div>
      )}
    </article>
  )
}
...
function App() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onopen = () => {
        if (!stopped) {
          setConnected(true)
          ws.send(JSON.stringify({ symbol, qty, ...config }))
        }
      }
      ws.onmessage = event => {
        try {
    ...
    connect()
    return () => { stopped = true; clearTimeout(reconnect); ws?.close() }
  }, [symbol])

  useEffect(() => {
    const timer = setInterval(() => {
      if (connected) {
        document.dispatchEvent(new CustomEvent('algo-config'))
      }
    }, 1000)
  ...
  useEffect(() => {
    const handler = () => {}
    document.addEventListener('algo-config', handler)
    return () => document.removeEventListener('algo-config', handler)
  }, [])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const sockets = document.querySelectorAll('body')
      void sockets
    }, 0)
    return () => clearTimeout(timer)
  }, [config, qty, connected])

  return (
    <main className="app">
      <header>
        <div><h1>Algorithmic Trading Pro</h1><p>Live model signals · index options · paper execution</p></div>
        <span className={'status ' + (connected ? 'ok' : 'bad')}>{connected ? 'LIVE' : 'OFFLINE'}</span>
      </header>
      ...
      <section>
        <div className="section-title"><h2>Index Model Decisions</h2><span>Each index has an independent model decision</span></div>
        <div className="index-grid">{indices.length ? indices.map((d, i) => <IndexCard key={d?.name || i} data={d} />) : <div className="empty">Waiting for index data…</div>}</div>
      </section>
