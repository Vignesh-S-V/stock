...
function IndexCard({ data }) {
  ...
  return <article className="index-card">
    ...
    <div className="index-price-row"><span>Spot</span><strong>₹{fmt0(data?.price)}</strong></div>
    <div className="index-decision-row"><span>MODEL DECISION</span><strong className={'decision ' + cls(decision)}>{decision === 'BUY' ? '🟢 BUY / UP' : decision === 'SELL' ? '🔴 SELL / DOWN' : '🟡 HOLD / SIDEWAYS'}</strong></div>
...
function App() {
  ...
  return <main className="app">
    ...
    <section className="controls">
      ...
      <label>Confidence Threshold %<input type="number" min="50" max="100" value={config.threshold} onChange={e => sendConfig({ threshold: Number(e.target.value) })} /></label>
    ...
    <section className="hero">
      ...
      <div><small>CALIBRATED CONFIDENCE</small><b>{signal.confidence ?? '—'}%</b></div>
